...
import eu.squadd.testing.objectspopulator.api.DataProviderStrategy.Order;
import eu.squadd.testing.objectspopulator.common.AttributeStrategy;
import eu.squadd.testing.objectspopulator.common.ManufacturingContext;
...
import eu.squadd.testing.objectspopulator.typeManufacturers.TypeManufacturerUtil;
...
public class ScannerFactoryImpl implements ScannerFactory {
     ...
     * Application logger
     */
    private static final Logger LOG = LoggerFactory.getLogger(ScannerFactoryImpl.class);
     ...
     * </p>
     */
    private ScannerFactory externalFactory = NullExternalFactory.getInstance();
     ...
     */
    public ScannerFactoryImpl(DataProviderStrategy strategy) {
     ...
     *
     * @param externalFactory External factory to delegate production this
     * factory cannot handle
     */
    public ScannerFactoryImpl(ScannerFactory externalFactory) {
     ...
     * @param strategy The strategy to use to fill data
     */
    public ScannerFactoryImpl(ScannerFactory externalFactory, DataProviderStrategy strategy) {
    ...
    public <T> T populatePojo(T pojo, Type... genericTypeArgs) {
        ...
        try {
            ...
            throw new MockeryException(e.getMessage(), e);
        }
    }
     ...
     */
    @Override
    public ScannerFactory setClassStrategy(ClassInfoStrategy classInfoStrategy) {
     ...
     * @param factoryClass The factory class, which will be used for POJO
     * instantiation
     ...
     * @param genericTypeArgs The generic type arguments for the current generic
     * class instance
     *
     *
    ...
    private <T> T instantiatePojoWithFactory(
        ...
        for (Method candidateConstructor : declaredMethods) {
            ...
            parameterValues = getParameterValuesForMethod(candidateConstructor, pojoClass, manufacturingCtx, typeArgsMap, genericTypeArgs);
            try {
                @SuppressWarnings("unchecked")
                T retValue = (T) candidateConstructor.invoke(factoryInstance, parameterValues);
    ...
    private <T> T instantiatePojo(Class<T> pojoClass, ManufacturingContext manufacturingCtx,
        ...
        if (constructors.length == 0 || Modifier.isAbstract(pojoClass.getModifiers())) {
            ...
            try {
                retValue = instantiatePojoWithFactory(pojoClass, pojoClass, manufacturingCtx, typeArgsMap, genericTypeArgs);
            } catch (InstantiationException | IllegalAccessException | InvocationTargetException | ClassNotFoundException e) {
                LOG.debug("Couldn't create an instance for pojo: " + pojoClass + " with factory methods, will " + " try non-public constructors.", e);
            }
        ...
        if (retValue == null && constructors.length > 0) {
            ...
            for (Constructor<?> constructor : constructors) {
                try {
                    ...
                    retValue = tmp;
                    if (retValue != null) {
                        LOG.debug("Could create an instance with constructor: " + constructor);
                        break;
                    }
                } catch (InstantiationException | IllegalAccessException | InvocationTargetException | ClassNotFoundException | SecurityException | IllegalArgumentException e) {
                    LOG.debug("Couldn't create an instance for pojo: {} with constructor: {}. Will try with another one.", pojoClass, constructor, e);
                }
            }
        }
    ...
    private <T> T doManufacturePojo(Class<T> pojoClass, ManufacturingContext manufacturingCtx, Type... genericTypeArgs) {
        try {
            ...
            AttributeMetadata pojoMetadata = new AttributeMetadata(pojoClass, pojoClass, genericTypeArgs, declaringClass);
    ...
    private <T> T manufacturePojoInternal(Class<T> pojoClass, AttributeMetadata pojoMetadata,
        ...
        @SuppressWarnings("unchecked")
        T objectToReuse = (T) strategy.getMemorizedObject(pojoMetadata);
        if (objectToReuse != null) {
            LOG.debug("Fetched memorized object for {} with parameters {}", pojoClass, Arrays.toString(genericTypeArgs));
        ...
        if (null == retValue) {
            ...
            try {
                ...
                throw new MockeryException("Security exception while applying introspection.", e);
            }
        }
        if (retValue == null) {
            return getValueForAbstractType(pojoClass, pojoMetadata, manufacturingCtx, typeArgsMap, genericTypeArgs);
    ...
    private <T> T populatePojoInternal(T pojo, ManufacturingContext manufacturingCtx,
        ...
        if (pojo instanceof Collection && ((Collection<?>) pojo).isEmpty()) {
            ...
            AtomicReference<Type[]> elementGenericTypeArgs = new AtomicReference<Type[]>(ScannerConstants.NO_TYPES);
            Class<?> elementTypeClass = findInheritedCollectionElementType(collection, manufacturingCtx, elementGenericTypeArgs, typeArgsMap, genericTypeArgs);
        ...
        while (iter.hasNext()) {
            ...
            // that attribute.
            List<Annotation> pojoAttributeAnnotations = ScannerUtils.getAttributeAnnotations(attribute.getAttribute(), setter);
            ...
            if (null != attributeStrategy) {
                LOG.debug("The attribute: " + attribute.getName() + " will be filled using the following strategy: " + attributeStrategy);
                setterArg = TypeManufacturerUtil.returnAttributeDataStrategyValue(attributeType, attributeStrategy);
            } else {
                AtomicReference<Type[]> typeGenericTypeArgs = new AtomicReference<Type[]>(ScannerConstants.NO_TYPES);
                ...
                if (!(genericType instanceof GenericArrayType)) {
                    ...
                    typeArguments = ScannerConstants.NO_TYPES;
                }
        ...
        for (ClassAttribute readOnlyAttribute : classAttributes) {
            ...
            if (getter != null && !getter.getReturnType().isPrimitive()) {
                if (getter.getGenericParameterTypes().length == 0) {
                    ...
                    try {
                        fieldValue = getter.invoke(pojo, ScannerConstants.NO_ARGS);
                    ...
                    if (fieldValue != null) {
                        ...
                        if (depth <= strategy.getMaxDepth(fieldClass)) {
                            ...
                            LOG.warn("Loop in filling read-only field {} detected.", getter);
                        }
                    }
                } else {
     ...
     * manufactured
     * @param genericAttributeType The generic type of the attribute for which a
     * value is being manufactured
     ...
     * <li>If an invalid value was set for a precise value in an annotation and
     * such value could not be converted to the desired type</li>
     * </ul>
     * @throws ClassNotFoundException If class being manufactured cannot be
     * loaded
     *
     */
    private Object manufactureAttributeValue(Object pojo,
        ...
        if (attributeType != genericAttributeType && Object.class.equals(attributeType) && genericAttributeType instanceof TypeVariable) {
            AtomicReference<Type[]> elementGenericTypeArgs = new AtomicReference(ScannerConstants.NO_TYPES);
        ...
        if (attributeValue == null) {
            ...
            if (depth <= strategy.getMaxDepth(pojoClass)) {
                ...
                attributeValue = resortToExternalFactory(manufacturingCtx, "Loop in {} production detected. Resorting to {} external factory", realAttributeType, genericTypeArgsAll);
            }
        }
     ...
     * @throws MockeryException An exception occurred while resolving the
     * collection
    ...
    private Collection<? super Object> resolveCollectionValueWhenCollectionIsPojoAttribute(
        ...
        if (null != pojo && null != attributeName) {
            ...
            defaultValue = ScannerUtils.getFieldValue(pojo, attributeName);
        }
        ...
        if (null != defaultValue && (defaultValue.getClass().getModifiers() & Modifier.PRIVATE) == 0) {
            ...
            @SuppressWarnings("unchecked")
            Class<Collection<Object>> collectionType = (Class<Collection<Object>>) attributeMetadata.getAttributeType();
        ...
        try {
            ...
            if (ArrayUtils.isEmpty(attributeMetadata.getAttrGenericArgs())) {
                typeClass = findInheritedCollectionElementType(retValue, manufacturingCtx, elementGenericTypeArgs, typeArgsMap, attributeMetadata.getAttrGenericArgs());
            } else {
            ...
            throw new MockeryException(RESOLVING_COLLECTION_EXCEPTION_STR, e);
     ...
     * @param manufacturingCtx the manufacturing context
     * @param elementGenericTypeArgs parameter to return generic arguments of
     * collection element
     * @param typeArgsMap a map relating the generic class arguments ("&lt;T,
     * V&gt;" for example) with their actual types
     * @param genericTypeArgs The generic type arguments for the current generic
     ...
     */
    private Class<?> findInheritedCollectionElementType(
        ...
        Type[] elementGenericArgs = ArrayUtils.addAll(elementGenericTypeArgs.get(), genericTypeArgs);
     ...
     * @param attributeName The attribute name of collection in enclosing POJO
     * class
     * @param collection The Collection to be filled
    ...
    private void fillCollection(ManufacturingContext manufacturingCtx,
        ...
        // we use it
        Holder<AttributeStrategy<?>> elementStrategyHolder = new Holder();
        Holder<AttributeStrategy<?>> keyStrategyHolder = null;
        Integer nbrElements = TypeManufacturerUtil.findCollectionSize(strategy, annotations, collectionElementType, elementStrategyHolder, keyStrategyHolder);
        ...
        try {
            ...
            for (int i = collection.size(); i < nbrElements; i++) {
                ...
                if (null != elementStrategy
                    ...
                    element = manufactureAttributeValue(collection, manufacturingCtx, collectionElementType, collectionElementType, annotations, attributeName, nullTypeArgsMap, genericTypeArgs);
                }
     ...
     * </ul>
     *
     * @throws MockeryException If an error occurred while creating the Map
     * object
     */
    private Map<? super Object, ? super Object> resolveMapValueWhenMapIsPojoAttribute(
        ...
        } catch (InstantiationException | IllegalAccessException | SecurityException | ClassNotFoundException | InvocationTargetException e) {
            throw new MockeryException(MAP_CREATION_EXCEPTION_STR, e);
        }
        return retValue;
    }
    ...
    private MapArguments findInheretedMapElementType(Map<Object, Object> map,
        ...
        while (typeParams.length < 2) {
            for (Type genericIface : mapClass.getGenericInterfaces()) {
                Class<?> clazz = TypeManufacturerUtil.resolveGenericParameter(genericIface, typeArgsMap, elementGenericTypeArgs);
            ...
            if (type != null) {
                Class<?> clazz = TypeManufacturerUtil.resolveGenericParameter(type, typeArgsMap, elementGenericTypeArgs);
                if (Map.class.isAssignableFrom(clazz)) {
                    typeParams = elementGenericTypeArgs.get();
                    mapClass = clazz;
                    continue;
        ...
        AtomicReference<Type[]> keyGenericTypeArgs = new AtomicReference(ScannerConstants.NO_TYPES);
        ...
        Type[] keyGenericArgs = ArrayUtils.addAll(keyGenericTypeArgs.get(), genericTypeArgs);
        Type[] elementGenericArgs = ArrayUtils.addAll(elementGenericTypeArgs.get(), genericTypeArgs);
    ...
    private void fillMap(MapArguments mapArguments, ManufacturingContext manufacturingCtx)
        ...
        // we use it
        Holder<AttributeStrategy<?>> elementStrategyHolder = new Holder();
        Holder<AttributeStrategy<?>> keyStrategyHolder = new Holder();
    ...
    private Object getMapKeyOrElementValue(
        ...
        if (null != keyOrElementsArguments.getElementStrategy()
                && ObjectStrategy.class.isAssignableFrom(keyOrElementsArguments.getElementStrategy().getClass())
                && Object.class.equals(keyOrElementsArguments.getKeyOrValueType())) {
    ...
    /**
     * Given a method it manufactures and returns the parameter values required
     * to invoke it
     *
     ...
     * @throws InstantiationException If a problem occurred while instantiating
     * the object
     * @throws IllegalAccessException If a problem occurred while instantiating
     * the object
     * @throws InvocationTargetException If a problem occurred while
     * instantiating the object
     * @throws ClassNotFoundException If a problem occurred while instantiating
     * the object
     */
