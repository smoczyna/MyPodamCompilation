...
public class BeanValidationStrategy implements AttributeStrategy<Object> {
     ...
     * {@inheritDoc}
     */
    public Object getValue() throws MockeryException {
        ...
        if (null != (minAnno = findTypeFromList(annotations, Size.class))) {
            ...
            if (maxValue == Integer.MAX_VALUE) {
                maxValue = ScannerConstants.STR_DEFAULT_LENGTH;
            }

            long length = ScannerUtils.getLongInRange(minValue, maxValue);

            StringBuilder sb = new StringBuilder();
            while (sb.length() < length) {
                sb.append(ScannerUtils.getNiceCharacter());
            }
            return sb.toString();
